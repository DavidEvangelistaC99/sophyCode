...
def main(args):
    ...
    if not conf['usrp_tx']['enable_2']: # One Pulse
        ...
        #pulse_1_width=ipp*1e6*dc*1e-2
        pulse_1_repetitions = N #### conf['usrp_tx']['repetitions_1']
        pulse_2_width = 0
        ...
        if conf['usrp_tx']['code_type_1'] != 'None':
            ...
            A = 1.0
            ipp = 400.0e-6
            dc = 1.0      # Consideramos el SR RX
            sr_tx = 20.0e6
            sr_rx = 2.5e6
            fc = 0.0
            bw = 1.0e6

            pulse_1_width=ipp*1.0e6*dc*1.0e-2
        ...
        if '1' in args.pulses:
            ...
            if conf['usrp_tx']['code_type_1'] != 'None':
                ...
                A1 = 1.0
                ipp1 = 400.0e-6
                dc1 = 1.0      # Consideramos el SR RX
                sr_tx1 = 20.0e6
                sr_rx1 = 2.5e6
                fc1 = 0.0
                bw1 = 1.0e6

                pulse_1_width=ipp1*1.0e6*dc1*1.0e-2

                chirp_tx_1, _ = modf.chirpMod(A1, ipp1, dc1, sr_rx1, sr_rx1, fc1, bw1, t_d = 0, window = 'B', mode_f = 0)
                # chirp_tx_1 = modf.chirpModUnion(ipp, sr_rx, sr_rx, A, A, 14.6, 0.4, 0.75e6, 0.0, 1.5e6, 0.0, 'B', 'R')
        ...
        if '2' in args.pulses:
            ...
            if conf['usrp_tx']['code_type_2']:
                ...
                A2 = 1.0
                ipp2 = 400.0e-6
                dc2 = 12.0      # Consideramos el SR RX
                sr_tx2 = 20.0e6
                sr_rx2 = 2.5e6
                fc2 = 0.0
                bw2 = 1.0e6

                pulse_2_width=ipp2*1.0e6*dc2*1.0e-2

                chirp_tx_2, _ = modf.chirpMod(A2, ipp2, dc2, sr_rx2, sr_rx2, fc2, bw2, t_d = 0, window = 'B', mode_f = 0)
                # chirp_tx_1 = modf.chirpModUnion(ipp, sr_rx, sr_rx, A, A, 14.6, 0.4, 0.75e6, 0.0, 1.5e6, 0.0, 'B', 'R')
